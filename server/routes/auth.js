...
router.post('/register', async (req, res) => {
    try {
        ...
        if (user) {
            if (user.email === email) return res.status(400).json({ error: 'Email 已被註冊' });
            if (user.username === username) return res.status(400).json({ error: '使用者名稱已被使用' });
        }
...
router.post('/resend-verification', async (req, res) => {
    try {
        ...
        if (!user) {
            // To prevent email enumeration, we can return success or a generic message.
            // But for UX, we often return "User not found" if security is less critical than usability in early stage.
            // Following current pattern:
            return res.status(404).json({ error: '找不到此 Email 的使用者' });
        }

        if (user.isVerified) {
            return res.status(400).json({ error: '此帳號已驗證，請直接登入' });
        }
        ...
        const subject = 'chat-app - 重新發送驗證碼';
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        // Auto-verify user if they successfully reset password (proof of email access)
        user.isVerified = true;
